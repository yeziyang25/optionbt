...
def cashflow_period_report(_detailed:pd.DataFrame, rebal_dates:list):
    ...
    _detailed['date_sec'] = pd.to_datetime(_detailed['date']).dt.strftime("%Y%m%d") + "_" + _detailed['sec_id']
    ...
    mv_col = _detailed.groupby(by=['date'], group_keys=True)[["close_mv"]].apply(sum).reset_index()
    _detailed["security_wt"] = _detailed["close_mv"]/_detailed['date'].map(dict(zip(mv_col["date"], mv_col["close_mv"])))
    _detailed["weighted_moneyness"] = _detailed["security_wt"].astype(float)*_detailed["moneyness"].replace("", 0).astype(float)
    ...
    equities_pricing_dict = {}
    for i_sec_id in np.unique(equities_pricing["sec_id"]).tolist():
        i_pricing = equities_pricing[equities_pricing["sec_id"]==i_sec_id]
        i_pricing_tr = common.total_return_calc(i_pricing, 'eod_price', 'dvd_rate')
        i_pricing_dict = dict(zip(pd.to_datetime(i_pricing_tr['date']).dt.strftime("%Y%m%d") + "_" + i_pricing_tr["sec_id"], i_pricing_tr["total_return_price"]))
        equities_pricing_dict = {**equities_pricing_dict, **i_pricing_dict}
    ...
    for idx, row in summary.iterrows():
        ...
        if not _detailed[(pd.to_datetime(_detailed['date']).dt.date == row['end_date']) & (_detailed['security_type'].str.contains('option')) & (_detailed['cash_inflow'] > 0)].empty:
            summary.loc[idx, 'moneyness'] = _detailed[(pd.to_datetime(_detailed['date']).dt.date == row['start_date']) & (_detailed['security_type'].str.contains('option')) & (_detailed['cash_inflow'] > 0)]['weighted_moneyness'].sum()

        # calculate basket return
        sub_basket = _detailed[(pd.to_datetime(_detailed['date']).dt.date == row['start_date'])]
        sub_basket = sub_basket[sub_basket["security_type"]=="equity"]
        sub_basket["start_tr_p"] = sub_basket["date_sec"].map(equities_pricing_dict)
        sub_basket["end_dt"] = row['end_date']
        sub_basket["end_dt_sec"] = pd.to_datetime(sub_basket["end_dt"]).dt.strftime("%Y%m%d") + "_" + sub_basket['sec_id']
        sub_basket["end_tr_p"] = sub_basket["end_dt_sec"].map(equities_pricing_dict)
        summary.loc[idx, 'basket_return'] = ((sub_basket["end_tr_p"]/sub_basket["start_tr_p"]-1)*sub_basket["security_wt"]).sum()
    ...
    return summary[['start_date', 'end_date', 'period_return', 'basket_return', 'option_prem_return', 'buyback_return', 'other_return', 'option_port_return', 'option_trading_return',
                    ...
                    'option_premium', 'option_buyback', 'net_premium', 'portfolio_start_mv', 'portfolio_end_mv', 'period_less_premium', '3m_rolling_return', '6m_rolling_return', 'moneyness', 'avg_moneyness']]
