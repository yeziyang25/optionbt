...
from data_download.weekly_tmx_option_downloader import run_weekly_tmx_download
...
class custom_option():
    def __init__(self, ticker:str, conn, rebal_dates, call_put):
        self.ticker = ticker
        self.opt_underlying_ticker = "RCI CN"
        self.conn = conn
...
if __name__ == '__main__':
    start_date = dt.datetime(2018, 1, 16)
    ...
    pct_otm_limit = 0.05
    DTM = 28
    strike_mode = "abs_closest" #round_up, round_down, abs_closest 

    # download dataframe
    ticker_lis = ["RCI"]
    run_weekly_tmx_download(
        ticker_lis=ticker_lis,
        start_date=start_date,
        end_date=end_date,
        DTM = DTM,
        pct_otm_target = pct_otm_target,
        strike_mode = strike_mode,
        call_put=call_put,
        pct_otm_limit=pct_otm_limit
    )

    option_rebal_dates = option_dates_customized(start_date, common.tsx_holidays(), end_date, DTM)
    ...
    for ticker in ticker_list:
        ...
        opt_cls.generate_custom_list(pct_otm=pct_otm_target)
        opt_cls.save_custom_file(cur_dir, "_1_test")
