...
def run_backtest(backtestname:str, end_date:dt.datetime, model:str, tenor:int, equity_rebal_rule:str, target_yield: int, holidays, timestamp:dt.datetime=dt.datetime.now(), save_folder:str=f"{cur_dir}\\output", optional_des:str=""):
    ...
    if model == 'btbuilder':
        if target_yield == -1:
            ...
            coverage_ratio = pd.DataFrame(list(coverage.items()), columns = ["Roll Day", "Coverage Ratio on 25% leverage"])
...
if __name__ == "__main__":
    ...
    end_date = dt.datetime(2025, 9, 19)
    ...
    run_backtest(backtestname, end_date, "btbuilder", 6, 'Q', 0.15, data_library.tsx_holidays(), timestamp=dt.datetime.now(), optional_des=f"_10_20_OTM",) # "-1" means we don't have a target yield and will use the fixed coverage ratio in config